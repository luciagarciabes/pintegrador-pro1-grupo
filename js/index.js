...
let loMasVisto= `https://api.themoviedb.org/3/trending/all/day?api_key=${api_key}`
...
.then(function (data) {
    ...
    for (let i = 0; i < 5; i++) {
      contenido2+= `<li class="cada_titulo">
                        <a href="./detail-serie.html"><img class="imagenes_home" src="" alt=" ${series[i].name}"   
                            height="250px">
                            <ul class="lista_anidada">
...
.then(function (data) {
    ...
    for (let i = 0; i < 5; i++) {
       contenido3+=  `<li class="cada_titulo">
                            <a href="./detail-serie.html"> <img class="imagenes_home" src="./img/friends.webp" alt="${trending[i].original_title}"
                                height="250px">
                                <ul class="lista_anidada">
                                <li class="li_piedefoto"> ${trending[i].original_title}</li>
                                <li class="li_piedefoto">Estreno:  ${trending[i].release_date} </li>
                                <li class="vermas"> Ver más </li>
                                </ul>
                            </a>
                      </li>`
